...
# Kept in sync with ha/automations/* — the simulator must emit exactly what
# the automations expect or it won't exercise the same code paths a real
# device would.
...
def prepare_renderer_inputs(triplet: dict, items: dict[str, dict]) -> dict:
    ...
    # Smart pill body comes from the summary item's YAML sidecar (the field
    # generated by the smart-pill batch and committed alongside each text).
    # The runtime path is currently `news.json` — when the renderer/HA
    # pipeline gets wired to read smart_pill directly from corpus YAML, this
    # bridge can go away.
    sp_body = ""
...
class ReviewServer(http.server.BaseHTTPRequestHandler):
    ...
    # so existing HA automations (schedule, gesture_override, now_playing_override)
    # fire on their normal code paths. We shell out to
