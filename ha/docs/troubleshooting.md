...
`summary.smart_pill.body` field. (The earlier per-source news sensors
generated by `ha/scripts/generate_news_sensors.py` are legacy; the
script is still on disk but no longer feeds the panel.)
