> **STATUS — 2026-04-27**: not started; superseded for now by an interim implementation. `pairing/publish_today.py` ships a simple sequence-rotation picker (sort triplets by `sequence`, index by `(today − epoch).days % len(triplets)`) that runs daily at 06:00 via HA's `inkplate_06_00_publish_today_s_triplet` automation. There's no `pairings/{date}.json` per-date file, no flavor cadence, no seasonal theme hints, no recency store, no weekly batch pre-generation, no year-ahead buffer, no `corpus pair` subcommand, and no batch report — Tasks §1–10 of this proposal are unstarted. The interim works for production but is operator-uncontrollable. The full design here remains the target whenever ongoing-curation requirements justify it (e.g. "operator wants to preview the next month and tune flavor-cadence" or "fleet of devices needs per-installation pairings"). Until then, treat this proposal as a roadmap, not a near-term deliverable.
