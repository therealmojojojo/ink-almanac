## STATUS: archived as deprioritized

The firmware tier schedule (Morning 06:30, Midday 10:00, Evening 17:00,
Night 22:00) remains hardcoded in `firmware/src/clock_zones.cpp`. The
proposal's goal — runtime-configurable boundaries via HA helpers — was
not implemented. Schedule changes still require a firmware recompile.

Revisit if the schedule needs frequent tuning or per-deployment
variation. The proposal's design and HA-helper plan are preserved
intact in this archived folder.
